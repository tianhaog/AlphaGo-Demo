...
class MeaninglessMove(Exception): pass
...
class Position():
    ...
    def is_move_filling_eyes(self, move):
        if is_eyeish(self.board, move):
            neighbor_group_id = self.lib_tracker.group_index[NEIGHBORS[move][0]]
            neighbor_group = self.lib_tracker.groups[neighbor_group_id]
            if self.to_play == neighbor_group.color:
                return True
        return False
        # print('checking meaningless move: ', move)
        # for n in NEIGHBORS[move]:
        #     print('n: ', n)
        #     neighbor_group_id = self.lib_tracker.group_index[n]
        #     print('neighbor_group_id: ', neighbor_group_id)
        #     if neighbor_group_id == MISSING_GROUP_ID:
        #         return False
        #     neighbor_group = self.lib_tracker.groups[neighbor_group_id]
        #     print('neighbor_group: ', neighbor_group)
        #     print('neighbor_group.color: ',neighbor_group)
        ...
        #     if neighbor_group.color != self.to_play:
        #         return False
        # for m in DIAGONALS[move]:
        #     diagonal_group_id = self.lib_tracker.group_index[m]
        #     if diagonal_group_id == MISSING_GROUP_ID:
        #         continue
        #     diagonal_group = self.lib_tracker.groups[diagonal_group_id]
        #     if diagonal_group.color != self.to_play:
        #         oppo_count += 1
        #         if oppo_count > 1:
        #             return False
        # return True
    ...
    def play_move(self, c, color=None, mutate=False):
        ...
        # if len(self.recent) > 150 and self.recent[-1] == gtp.PASS:
        ...
        if c is None:
            pos = pos.pass_move(mutate=mutate)
            return pos
        if self.n > 150 and self.is_move_filling_eyes(c):
            # print('Meaningless move:', c)
            raise IllegalMove()
        if not self.is_move_legal(c):
            # print('illgeal move:',c)
            raise IllegalMove()
